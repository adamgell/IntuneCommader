...
public class ProfileService
{
    ...
    public async Task LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_encryption is not null && raw.StartsWith(EncryptedMarker))
        {
            ...
            {
                // Decryption failed (keys rotated, corrupted, etc.). Keep the app
                // readable, but remember the failure so a later save cannot silently
                // overwrite the only copy of the encrypted profile store.
                System.Diagnostics.Debug.WriteLine(
                    $"[ProfileService] Profile decryption failed — blocking profile saves. Exception: {ex}");
                _loadError = new InvalidOperationException(
                    $"Could not decrypt saved tenant profiles at '{pathToLoad}'.", ex);
                _store = new ProfileStore();
                return;
            }
        }
    ...
    public async Task SaveAsync(CancellationToken cancellationToken = default)
    {
        if (_loadError is not null)
            throw new InvalidOperationException(
                "Refusing to overwrite profiles.json because the saved tenant profiles could not be decrypted.",
                _loadError);
